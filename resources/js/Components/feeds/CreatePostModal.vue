            ...
            <DialogPanel class="relative transform overflow-hidden rounded-xl bg-brand-50 px-4 pt-5 pb-4 text-left shadow-xl transition-all sm:my-8 w-full max-w-full sm:max-w-2xl sm:p-6">
              <div>
                ...
                <div class="mt-3 text-center sm:mt-5">
                  <form action="#" class="bg-brand-50 rounded-xl mb-4">
                    <TabGroup v-slot="{ selectedIndex }">
                      <TabPanels class="mt-2">
                        <TabPanel class="-m-0.5 rounded-xl p-0.5">
                          <label for="post" class="sr-only">Post</label>
                          <div>
                            <textarea @click="open = true" rows="6" name="post" id="post" class="block w-full rounded-xl border-brand-300 shadow-sm focus:border-brand-500 focus:ring-brand-500 sm:text-sm" placeholder="Write a new post..." />
                          </div>
                        </TabPanel>
                      </TabPanels>
                    </TabGroup>
              ...
              <div class="flex flex-row justify-end gap-2">
                ...
                <AppButton @click="open = false" :icon="PaperAirplaneIcon" color="brand">Post</AppButton>
              </div>
            </DialogPanel>
          </TransitionChild>
        </div>
